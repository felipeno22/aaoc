...
<form  id="idFormLegis" name="idFormLegis"  onsubmit="return insert_update_legis('insert')" class="formLegis" action="/mylegislacao/insert" method="post" enctype="multipart/form-data">
	

	<fieldset>
		
		<legend>Cadastrar Legislação</legend>

	
	
  <?php if( $error!= ''  ){ ?>
  	<?php if( $error== 'deu certo'  ){ ?>
  			<div class="alert alert-success" role="alert">Legislação cadastrada com sucesso!!!</div>
  	<?php }else{ ?>
 ...
 <?php } ?>	
			...
			<input type="text" id="legislacao" name="legislacao" required><br>
